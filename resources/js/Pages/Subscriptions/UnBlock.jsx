...
const UnBlockSubscription = () => {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const response = await ApiService.indexBlocked();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await ApiService.unblock(formData);
      if (response.data.success) {
  ...
  if(clients.length === 0){
    return <NoDataToShow
            heading="No Blocked Clients"
            description="There are currently no blocked subscriptions to manage." />
  }
...
export default UnBlockSubscription;